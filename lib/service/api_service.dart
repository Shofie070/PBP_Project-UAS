...
class ApiService {
  ...
  static List<Product> getMockProducts() {
    return [
      Product(id: 1, name: 'Kaos Polos Hitam', price: 75000, image: 'assets/images/Kaos1.png', category: 'Kaos'),
      Product(id: 2, name: 'Hoodie Urban Navy', price: 250000, image: 'assets/images/Hoodie1.png', category: 'Hoodie'),
      Product(id: 3, name: 'Kaos Graphic Art', price: 120000, image: 'assets/images/Kaos2.png', category: 'Kaos'),
      Product(id: 4, name: 'Topi Snapback', price: 50000, image: 'assets/images/hoodie2.png', category: 'Aksesoris'),
    ];
  }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<Product> products = data.map((json) {
          ...
          String rawCategory = (json['category'] ?? '').toString().toLowerCase();
          String title = (json['title'] ?? '').toString().toLowerCase();
          String appCategory = 'Lainnya'; // Default

          // Deteksi Sepatu
          if (rawCategory.contains('shoe') || rawCategory.contains('sneaker') || title.contains('shoe') || title.contains('sneaker')) {
            appCategory = 'Sepatu';
          }

          // Deteksi Celana (pants/jeans/trouser)
          else if (rawCategory.contains('pant') || rawCategory.contains('jean') || rawCategory.contains('trouser') || title.contains('pants') || title.contains('jeans')) {
            appCategory = 'Celana';
          }

          // Deteksi Pakaian / Kaos (shirt/top/t-shirt)
          else if (rawCategory.contains('shirt') || rawCategory.contains('top') || title.contains('t-shirt') || title.contains('shirt')) {
            appCategory = 'Pakaian';
          }

          // Deteksi Aksesoris (Watch, Jewelry, Bag, Sunglasses)
          else if (rawCategory.contains('watch') || rawCategory.contains('jewel') ||
                   rawCategory.contains('bag') || rawCategory.contains('sunglass') || title.contains('watch') || title.contains('jam')) {
            appCategory = 'Aksesoris';
          }

          // Deteksi Hoodie explicitly by title/keywords
          if (title.contains('hoodie') || title.contains('sweat') || title.contains('jacket')) {
            appCategory = 'Hoodie';
          }
